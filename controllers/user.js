...
module.exports.updateAddress = async (req, res) => {
    ...
    try {
        ...
        const { street, city, province, country, postalCode, pinLocation, isDefault } = req.body;

        // Preserve the current value of isDefault
        const currentIsDefault = user.addresses[addressIndex].isDefault;
        ...
        user.addresses[addressIndex] = {
            ...
            postalCode,
            pinLocation,
            isDefault: currentIsDefault // Set the isDefault to its current value
        };
...
module.exports.updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.mobileNo) {
            updatedFields.mobileNo = req.body.mobileNo;
        }

        const updatedUser = await User.findByIdAndUpdate(userId, updatedFields, { new: true })
        .then((result) => {
            return res.status(201).send({ message: "Profile updated successfully", user: result })
        })
        .catch((err) => {
            console.error("Error in Save: ", err);
            return res.status(500).send({ error: 'Error in updating' });
        });
...
module.exports.getAllUsers = (req, res) => {
    ...
    console.error("Error in finding the user: ", err)
        return res.status(500).send({ error: 'Error finding all users'})
    });
};