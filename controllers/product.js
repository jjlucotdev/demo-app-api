...
module.exports.addProduct = async (req, res) => {
  try {
    const { name, description, price, categoryId, productImage } = req.body;
    ...
    return res.status(201).send({ newProduct: savedProduct });
  } catch (err) {
...
module.exports.featureProduct = async (req, res) => {
    const productId = req.params.productId;
    const featuredProduct = { isFeatured: true };

    return await Product.findByIdAndUpdate(productId, featuredProduct)
    .then(product => {
        if (product) {
            return res.status(200).send({ message: 'Product featured successfully', featuredProduct: product });
        } else {
            return res.status(404).send({ error: 'Product not found' });
        }
    })
    .catch(err => {
        ...
        return res.status(500).send({ error: 'Failed to feature product' });
    })
};

module.exports.dontFeatureProduct = async (req, res) => {
    const productId = req.params.productId;
    const unfeaturedProduct = { isFeatured: false };

    return await Product.findByIdAndUpdate(productId, unfeaturedProduct)
    .then(product => {
        if (product) {
            return res.status(200).send({ message: 'Product removed from featured successfully', unfeaturedProduct: product });
        } else {
            return res.status(404).send({ error: 'Product not found' });
        }
    })
    .catch(err => {
        console.error("Error removing highlight product: ", err);
        return res.status(500).send({ error: 'Failed to remove highlight product' });
    })
};
